...
mkdir -p dist && docker run --detach=true --name=ngo-login-client-builder -e "OPENFACT_SYNC_API_URL=http://api.openshift.io/api/" -e JENKINS_URL -e GIT_BRANCH -e "CI=true" -e GH_TOKEN -e NPM_TOKEN -t -v $(pwd)/dist:/dist:Z ngo-login-client-builder
...
docker exec ngo-login-client-builder ./fix-git-repo.sh

# Build almigty-ui
docker exec ngo-login-client-builder npm install

## Build prod
docker exec ngo-login-client-builder npm run build

## Exec functional tests
docker exec ngo-login-client-builder ./run_unit_tests.sh
...
## Exec functional tests
docker exec ngo-login-client-builder ./run_functional_tests.sh
...
if [ $? -eq 0 ]; then
  ...
  set +e
  # Publish to npm
  docker exec ngo-login-client-builder npm run semantic-release
  if [ $? -eq 0 ]; then
